...
template <int BLOCK_DIM, int TILE_K>
__global__ void gemm_tiled_kernel(const float* A, const float* B, float* C,
                                   int M, int N, int K) {
    __shared__ float A_tile[BLOCK_DIM][TILE_K];
    __shared__ float B_tile[TILE_K][BLOCK_DIM];

    int const C_col_idx = blockIdx.x * BLOCK_DIM + threadIdx.x;
    int const C_row_idx = blockIdx.y * BLOCK_DIM + threadIdx.y;
    int const num_tiles = (K + TILE_K - 1) / TILE_K;

    float value = 0.0f;

    for (int tileIdx = 0; tileIdx < num_tiles; ++tileIdx) {
        for (int k = threadIdx.x; k < TILE_K; k += BLOCK_DIM) {
            int global_k = tileIdx * TILE_K + k;
            A_tile[threadIdx.y][k] =
                (C_row_idx < M && global_k < K) ? A[C_row_idx * K + global_k] : 0.0f;
        }

        for (int k = threadIdx.y; k < TILE_K; k += BLOCK_DIM) {
            int global_k = tileIdx * TILE_K + k;
